...
interface EditPackageFormProps {
  travelPackage: {
    ...
    price: any; // Decimal type or number
    duration: number;
...
export function EditPackageForm({ travelPackage, destinations }: EditPackageFormProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [imagePreview, setImagePreview] = useState<string | null>(travelPackage.featuredImage);
  ...
  const {
    ...
    defaultValues: {
      ...
      fullDescription: travelPackage.fullDescription,
      featuredImage: travelPackage.featuredImage || "",
      status: travelPackage.status as any,
    },
  });
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const res = await updatePackageAction(travelPackage.id, {
        ...data,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Edit Travel Package</h2>
      </div>
