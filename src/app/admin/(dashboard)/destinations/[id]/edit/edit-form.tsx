...
export function EditDestinationForm({ destination }: EditDestinationFormProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [imagePreview, setImagePreview] = useState<string | null>(destination.featuredImage);
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (uploadRes.success && uploadRes.url) {
          ...
          toast.error("Failed to upload image, saving with previous media.");
        }
      }

      const res = await updateDestinationAction(destination.id, {
        ...data,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Edit Destination</h2>
      </div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                {errors.country && (
                  <p className="text-xs text-red-500 font-semibold">{errors.country.message}</p>
                )}
              </div>
              <div className="space-y-2">
                <Label htmlFor="city">City/Region</Label>
                <Input id="city" placeholder="e.g. Bali" {...register("city")} />
                {errors.city && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Input id="category" placeholder="e.g. Island, City, Mountain" {...register("category")} />
                {errors.category && (
          ...
          <CardContent className="space-y-4">
            {imagePreview ? (
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-lg p-12 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-slate-500 mb-4">Select a photo for the destination card header</p>
